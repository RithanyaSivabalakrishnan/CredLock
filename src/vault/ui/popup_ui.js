...
// Separate cores — CVV pad (max 4 digits) and OTP pad (max 8 digits)
const padCoreOtp = new VirtualPadCore(8, 'otp');
const padCoreCvv = new VirtualPadCore(4, 'cvv');

const padViewMain = new VirtualPadView(padCoreOtp,
  document.getElementById('sv-virtual-pad-mount'), 'otp');
const padViewAdd  = new VirtualPadView(padCoreCvv,
  document.getElementById('sv-cvv-pad-mount'), 'cvv');

const noiseLayer = new UiNoiseLayer(document.getElementById('sv-noise-canvas'));

// Wire CVV pad → fills the CVV input field as user presses keys
padCoreCvv.on('input', (masked) => {
  const cvvField = document.getElementById('sv-field-cvv');
  if (cvvField) {
    cvvField.value = padCoreCvv.getValue();
    cvvField.dispatchEvent(new Event('input', { bubbles: true }));
  }
});
padCoreCvv.on('cleared', () => {
  const cvvField = document.getElementById('sv-field-cvv');
...
document.getElementById('sv-biometric-unlock-btn-removed')
  ?.addEventListener('click', async () => {
